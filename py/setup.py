...
"""Build unichecksum._core, a Cython extension over the UniChecksum C ABI.
...
PKG_DIR = os.path.join(HERE, "unichecksum")
VENDOR_DIR = os.path.join(HERE, "_nimsrc")
NIMBLE_FILE = "UniChecksum.nimble"
...
if sys.platform == "win32":
    LIB_NAME, BUNDLED = "UniChecksum.lib", False
    LINK_ARGS, NIMBLE_TASK = [], "clibMsvc"
elif sys.platform == "darwin":
    LIB_NAME, BUNDLED = "libUniChecksum.dylib", True
    LINK_ARGS, NIMBLE_TASK = ["-Wl,-rpath,@loader_path"], "clib"
else:
    LIB_NAME, BUNDLED = "libUniChecksum.so", True
...
def nim_project_dir():
    """Where UniChecksum.nimble lives: the real repo root in a normal
...
def ensure_lib_built():
    ...
    try:
        ...
        raise SystemExit(
            "setup.py: `nimble` not found on PATH. Building unichecksum from "
...
# The sdist ships the pre-transpiled unichecksum/_core.c, not the .pyx (Cython
...
# needing no Cython.
pyx = os.path.join("unichecksum", "_core.pyx")
ext = Extension(
    "unichecksum._core",
    sources=[pyx if os.path.exists(os.path.join(HERE, pyx)) else os.path.join("unichecksum", "_core.c")],
    include_dirs=[INCLUDE],
    library_dirs=[LIB_DIR],
    extra_link_args=LINK_ARGS,
    libraries=["UniChecksum"],
)
...
setup(
    ...
    package_data={"unichecksum": [LIB_NAME] if BUNDLED else []},
    exclude_package_data={"unichecksum": ["_core.c"]},
    zip_safe=False,
)
