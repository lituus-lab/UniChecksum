...
suite "one version, seven copies":
  let manifest = valueOf("UniChecksum.nimble", "version", "\"", "\"")
  ...
  test "the Nim constant agrees":
    check UniChecksumVersion == manifest

  test "the C header agrees, macros and string alike":
    let parts = manifest.split('.')
    check valueOf("include/UniChecksum.h", "UNICHECKSUM_VERSION_MAJOR", " ",
        "") == parts[0]
    check valueOf("include/UniChecksum.h", "UNICHECKSUM_VERSION_MINOR", " ",
        "") == parts[1]
    check valueOf("include/UniChecksum.h", "UNICHECKSUM_VERSION_PATCH", " ",
        "") == parts[2]
    check valueOf("include/UniChecksum.h", "define UNICHECKSUM_VERSION ", "\"",
        "\"") == manifest

  test "the C ABI reports it":
    # Read from the source rather than called: this suite links no C library.
    check valueOf("src/UniChecksum/c_api.nim", "UniChecksumVersionC", "\"",
        "\"") == manifest
  ...
  test "the Python test expects it":
    check valueOf("py/tests/test_checksums.py", "unichecksum.version()", "\"",
        "\"") == manifest
